...
import { makeSaveV11 } from "./fixtures/save-v11";
...
describe("cloud save API", () => {
  it("aceita os 1.189 capítulos da Bíblia e mantém um teto defensivo", () => {
    const completeBible = makeSaveV11();
    ...
    expect(validateSavePayload(JSON.stringify(completeBible), 11, 11).bytes).toBeGreaterThan(1_189);

    study.progresso.capitulosLidos = Array.from(
      { length: 1_201 },
      (_, index) => `C${String(Math.floor(index / 19)).padStart(2, "0")}:${index % 19 + 1}`,
    );
    expect(() => validateSavePayload(JSON.stringify(completeBible), 11, 11)).toThrow();
  });

  it("aceita o save v11 realista com sentinela -1 e rejeita relógio futuro", async () => {
    const payload = JSON.stringify(makeSaveV11());
    expect(validateSavePayload(payload, 11, 11).bytes).toBeGreaterThan(100);
    const expanded = makeSaveV11();
    expanded.graca = 100;
    ...
    expect(validateSavePayload(JSON.stringify(expanded), 11, 11).bytes).toBeGreaterThan(100);

    const invalidCosmetic = structuredClone(expanded);
    invalidCosmetic.cosmeticosAtivos = { titulo: "fundo_aurora" };
    expect(() => validateSavePayload(JSON.stringify(invalidCosmetic), 11, 11)).toThrow();
    const future = makeSaveV11();
  ...
  it("cria conta, grava, detecta conflito e mantém retries antigos idempotentes", async () => {
    const account = await createAccount();
    const firstPayload = JSON.stringify(makeSaveV11(20));
    ...
    const secondPayload = JSON.stringify(makeSaveV11(30));
    ...
    const conflict = await upload(account.sessionToken, 1, JSON.stringify(makeSaveV11(40)));
  ...
  it("mantém restore-previous idempotente mesmo depois de outra escrita", async () => {
    const account = await createAccount();
    await upload(account.sessionToken, 0, JSON.stringify(makeSaveV11(20)));
    await upload(account.sessionToken, 1, JSON.stringify(makeSaveV11(30)));
    ...
    await upload(account.sessionToken, 3, JSON.stringify(makeSaveV11(50)));
  ...
  it("recupera em segundo aparelho e o CAS impede overwrite silencioso", async () => {
    ...
    expect((await upload(account.sessionToken, 0, JSON.stringify(makeSaveV11(20)))).response.status).toBe(200);
    expect((await upload(recovered.sessionToken, 0, JSON.stringify(makeSaveV11(30)))).response.status).toBe(412);
  });
  ...
  it("scheduled limita snapshots a cinco por conta", async () => {
    const account = await createAccount();
    const payload = JSON.stringify(makeSaveV11(20));
